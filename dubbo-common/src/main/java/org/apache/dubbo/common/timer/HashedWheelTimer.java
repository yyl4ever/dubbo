...
public class HashedWheelTimer implements Timer {
     ...
     * 在定时任务进入到 timeouts 队列之前会先调用 start() 方法启动时间轮
     * @param task
     * @param delay
     * @param unit
     * @return
    ...
    public Timeout newTimeout(TimerTask task, long delay, TimeUnit unit) {
        ...
        // 根据 startTime 计算该定时任务的 deadline 字段，最后才能将定时任务封装成 HashedWheelTimeout 并添加到 timeouts 队列。
    ...
    private static final class HashedWheelTimeout implements Timeout {
        ...
        /**
         * 定时任务当前所处状态，可选的有三个，分别是 INIT（0）、CANCELLED（1）和 EXPIRED（2）。
         * 另外，还有一个 STATE_UPDATER 字段（AtomicIntegerFieldUpdater类型）实现 state 状态变更的原子性。
         */
    ...
    private static final class HashedWheelBucket {
         ...
         * 遍历双向链表中的全部 HashedWheelTimeout 节点。 在处理到期的定时任务时，会通过 remove() 方法取出，并调用其 expire() 方法执行；
